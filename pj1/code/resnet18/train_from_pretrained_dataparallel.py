...
# 配置日志记录的格式和级别
logging.basicConfig(filename=f'/share/home/zjy/code_repo/DATA620004-SS24/pj1/code/resnet18/logs/dataparallel_resnet18_from_pretrained_fc_lr_{args.fc_learning_rate}_pretrained_lr_{args.pretrained_learning_rate}_momentum_{args.momentum}_num_epochs_{args.num_epochs}.log', level=logging.INFO,
                    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')

# 设置TensorBoard日志目录
writer = SummaryWriter(f'./runs/dataparallel_resnet18_from_pretrained_fc_lr_{args.fc_learning_rate}_pretrained_lr_{args.pretrained_learning_rate}_momentum_{args.momentum}_num_epochs_{args.num_epochs}')
...
# 创建数据加载器
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=64*8, shuffle=True)
val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=64*8, shuffle=False)
