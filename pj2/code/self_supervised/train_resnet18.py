...
def train_simclr(model, dataset_name, dataloader, optimizer, scheduler, epochs=500):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % save_every == 0 or epoch == epochs - 1:
            torch.save(model.state_dict(), f'./ckpt/resnet18_simclr_{dataset_name}_epoch{epoch+1}.pth')
...
if __name__ == '__main__':
    ...
    writer = SummaryWriter(log_dir='./runs/SimCLR_experiment_resnet18')
    ...
    if dataset_name == 'cifar10':
        dataset = datasets.CIFAR100(root='/home/add_disk/zhangjinyu/dataset/cifar100', train=True, download=True, transform=ContrastiveLearningViewGenerator(data_transforms, 2))
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=256, num_workers=12, shuffle=True, pin_memory=True, drop_last=True)
    elif dataset_name == 'stl10':
        dataset = datasets.STL10(root='/home/add_disk/zhangjinyu/dataset/stl10', split='unlabeled', download=True, transform=ContrastiveLearningViewGenerator(data_transforms, 2))
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=256, num_workers=12, shuffle=True, pin_memory=True, drop_last=True)
